  ...
  useEffect(() => {
    const GetCoordinator = async () => {
      ...
      const response = await axios.get(
        "http://localhost:3001/teacher/getById/" + id
      );
          ...
          <div className="d-flex flex-column align-items-center mb-2">
            <div className="text-center">
              <p style={{ fontSize: "1.2rem" }}>
                Nombre completo: {coordinator.TEACHER_FIRSTNAME}{" "}
                {coordinator.TEACHER_LASTNAME}
              </p>
              <p style={{ fontSize: "1.2rem" }}>
                Cédula: {coordinator.TEACHER_IDNUMBER}
              </p>
              <p style={{ fontSize: "1.2rem" }}>
                Tipo: {coordinator.TEACHER_TYPE}
              </p>
              <p style={{ fontSize: "1.2rem" }}>
                Contrato:{" "}
                {coordinator.TEACHER_CONTRACTTYPE === "PLT"
                  ? coordinator.TEACHER_CONTRACTTYPE + "-Planta"
                  : coordinator.TEACHER_CONTRACTTYPE + "-Contratista"}
              </p>
              <p style={{ fontSize: "1.2rem" }}>
                Área: {coordinator.TEACHER_AREA}
              </p>
            </div>
          </div>
        </div>
